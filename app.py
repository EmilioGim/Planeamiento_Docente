...
# ============= Mostrar planificación y botón de descarga (si disponible) ==============
...
if st.session_state.get("plan_creada", False):
    ...
    # ===== Descargar el Word si hay plantilla y contexto =====
    if plantilla_archivo and st.session_state.contexto_docx:
        try:
            ...
            output_word.seek(0)
            st.download_button("Descargar planificación Word", output_word.getvalue(), "planificacion_generada.docx")
        except Exception as e:
            st.warning(f"Error al generar archivo Word: {e}")
    elif not plantilla_archivo:
        st.info("Si quieres descargar el Word, sube primero la plantilla .docx y luego vuelve a presionar 'Generar planificación'.")
