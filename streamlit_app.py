...
# =========== AUTENTICACIÓN ===========
CORRECT_PASSWORD = "TuContraseña123"  # Cambia esto por tu clave
...
for i in range(int(n)):
    with st.expander(f"Unidad {i+1}"):
        ...
        contenido = st.text_area(f"Contenido (una frase por línea) para Unidad {i+1}:", key=f"contenido_{i}")
...
if st.button("Generar planificación"):
    ...
    # SpaCy: modelo español (gestión automática)
    try:
        import spacy
        try:
