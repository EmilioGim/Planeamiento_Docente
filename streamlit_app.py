...
# =========== AUTENTICACIÓN ===========
CORRECT_PASSWORD = "TuContraseña123"  # Cambia esto en producción
...
if not st.session_state.authenticated:
    ...
    if st.button("Acceder"):
        if password == CORRECT_PASSWORD:
            ...
            st.warning("Contraseña incorrecta. Si no la conoces, contacta al administrador.")
    st.stop()

# =========== APLICACIÓN PRINCIPAL ===========
st.title("Planificador semanal académico interactivo")

# 1. Subida plantilla Word
plantilla_archivo = st.file_uploader("Sube tu plantilla .docx (opcional, antes de 'Generar planificación')", type="docx")
...
for i in range(int(n)):
    with st.expander(f"Unidad {i+1}"):
        ...
        contenido = st.text_area(
            f"Contenido (una frase por línea) para Unidad {i+1}:", key=f"contenido_{i}"
        )
...
if st.button("Generar planificación"):
    ...
    # SpaCy: modelo español (gestión automática, sin errores en Steam Cloud)
    try:
        import spacy
        try:
        ...
        st.warning("spaCy no está instalado (usará solo 'el contenido' como concepto): " + str(e))
        nlp = None
    ...
    if len(sesiones_normales_idx) < total_sesiones_normales:
        st.warning("Hay menos sesiones normales de las requeridas. Verifica feriados y eventos.")
...
if st.session_state.get("plan_creada", False):
    ...
    # ================= Segundo archivo Word: Resumen visual ==================
    ...
    for sesion in plan_semanal:
        ...
        doc_resumen.add_paragraph("") # separar sesiones
    ...
    if plantilla_archivo and st.session_state.contexto_docx:
        ...
        try:
            ...
            output_word.seek(0)
            st.download_button("Descargar Planificación Word (plantilla)", output_word.getvalue(), "planificacion_personalizada.docx")
        except Exception as e:
            st.warning(f"Error al generar archivo Word de plantilla: {e}")
    elif not plantilla_archivo:
        st.info("Para descargar la planificación con plantilla, sube tu .docx y vuelve a generar la planificación.")
